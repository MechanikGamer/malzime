...
- **Stundenlimit mit Firestore-Zaehler**: Globaler Analyse-Zaehler in Cloud Firestore mit konfigurierbarem Stundenlimit (Standard: 500/Stunde, zentral in `config.js`). Fail-open bei Firestore-Fehlern — die App bleibt nutzbar, auch wenn die Datenbank kurzzeitig nicht erreichbar ist. Erhoeht man das Limit per Admin-Boost, wird es nach Ablauf des Zeitfensters automatisch auf den Default zurueckgesetzt. Die Anzeige der stuendlichen Analysen (`hourlyTotal`) ist vom Limit-Zaehler (`count`) entkoppelt — Admin-Reset setzt nur den Limit-Zaehler zurueck, die echte Analysezahl bleibt korrekt.
